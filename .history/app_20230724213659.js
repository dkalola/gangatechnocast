...
const { getFirestore, collection, addDoc } = require("firebase/firestore"); // Use full Firestore package
const {
  getAuth,
  setPersistence,
...
app.get("/admin", ensureAuthenticated, (req, res) => {
  res.render("admin", { activePage: "admin", logout: true });
});
...
app.post("/submit-quote", async (req, res) => {
  ...
  try {
    ...
    const quotesCollection = app.collection("quotes");

    // Write the data to Firestore using the `add` method, which returns a promise
    const docRef = await quotesCollection.add(data);
